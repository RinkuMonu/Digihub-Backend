...
exports.initiatePayment = async (amount, orderId) => {
    const transactionId = `${orderId}-${Date.now()}`; // Unique transaction ID
    const paymentData = {
        merchantId,
        transactionId,
        amount: amount * 100, // Convert to paise
        currency: 'INR',
        callbackUrl: `${process.env.BASE_URL}/api/payment/paymentverification/${orderId}`,
    };
    ...
    try {
        const response = await axios.post(`${baseUrl}/v1/checkout`, paymentData, {
            headers: {
                ...
                'X-VERIFY': `${signature}###${secretKey.slice(-6)}`,
            },
        });

        return { transactionId, redirectUrl: response.data.redirectUrl };
    } catch (error) {
